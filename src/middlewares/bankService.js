...
const isPartner = (req, res, next) => {
  try {
    ...
    if (
      isNaN(ts) ||
      ts_now < ts ||
      ts_now - ts > parseInt(process.env.TIMEOUT) * 1000 // hiện tại đang set 5 phút ở .env (300)
    ) {
    ...
    const sig = md5(
      ts.toString() + JSON.stringify(content) + process.env.SERVICE_CODE
    )
    if (sig !== signature) {
    ...
    const partnerKey = fs.readFileSync(
      path.resolve(
        __dirname +
          `/../utils/partner-key/${bankName.toLowerCase()}-PublicKey.pem`
      ),
      "utf8"
    )
